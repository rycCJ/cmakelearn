...
#include "my_pro_project/cycle_list.hpp" // 包含我们要测试的库的头文件
...
TEST(Solution1Test, HasCycle)
{
    ...
    EXPECT_FALSE(sol.hasCycle(head1)); // 已修复：不会触发段错误

    // 测试用例2: 单节点无环
    ListNode *head2 = createNode(1);
    head2->next = nullptr;
    EXPECT_FALSE(sol.hasCycle(head2));
    delete head2; // 释放内存
    ...
    EXPECT_FALSE(sol.hasCycle(head3));
    // 释放内存（从尾到头释放）
    ...
    head4->next = head4; // 自环
    EXPECT_TRUE(sol.hasCycle(head4));
    ...
    EXPECT_TRUE(sol.hasCycle(head5));
